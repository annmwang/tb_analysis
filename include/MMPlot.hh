...
TCanvas* Plot_Track2D(string can, const MMClusterList* clusters = 0){
  ...
  p1->SetGrid();
  TPad *p2 = new TPad("p2", "", 0, 0.02, 1, 0.24);
  //p2->SetGrid();
  c1->Draw();
  c1->cd();
  p1->Draw();
  p2->Draw();
  TMultiGraph* mg = new TMultiGraph();
  TMultiGraph* mcharge = new TMultiGraph();

  int Nplane = 2;

  double min_x= 1000, max_x = -1;

  for(int ib = 0; ib < Nplane; ib++){
    std::vector<double> bary_x;
    ...
    for(int i = 0; i < Nclus; i++){
      int index = clusters->Get(i).MMFE8Index();
      if(index != ib)
        continue;
      
      double ch = clusters->Get(i).Channel();
      bary_x.push_back(ch);
      bary_z.push_back(index*60.7);
      for(int j = 0; j < clusters->Get(i).size(); j++){
        TVector3 p(clusters->Get(i)[j].Channel(),0.,index*60.7);
        //TVector3 p(clusters->Get(i)[j].Channel()*0.4,0.,index*60.7);
        vx.push_back(p.X());
        vz.push_back(p.Z());
        vpdo.push_back(clusters->Get(i)[j].Charge());
      }
    }

    double my_min_x = *std::min_element(vx.begin(), vx.end());
    double my_max_x = *std::max_element(vx.begin(), vx.end());
    ...
    TGraph* gr2 = new TGraph(N_bary,x1,z1);
    gr2->SetMarkerSize(2);
    ...
    TGraph* charge = new TGraph(N,x,pdo);
    charge->SetTitle("");
    //charge->SetFillStyle(4050);
    if (ib == 0){
      charge->SetFillColor(kRed-7);
      charge->SetLineColor(kRed-7);
      //charge->SetFillColorAlpha(kRed-7,0.6);
    }
    else {
      charge->SetFillColor(kBlue-7);
      charge->SetLineColor(kBlue-7);
      //charge->SetFillColorAlpha(kBlue-7,0.35);
    }
    //charge->SetBarWidth(0.3);
    //charge->Draw("AB same");
    mcharge->Add(charge);
    gPad->Update();
  }
  ...
  mg->GetXaxis()->SetRangeUser(min_x-3,max_x+3);
  mg->GetYaxis()->SetRangeUser(-4.,64.);
  //  mg->GetXaxis()->SetLabelSize(0);
  mg->GetXaxis()->SetTitle("strip number");
  mg->GetXaxis()->CenterTitle();
  p2->cd();
  mcharge->Draw("AB");
  mcharge->GetXaxis()->SetLabelSize(0);
  //mcharge->GetXaxis()->CenterTitle();
  mcharge->GetYaxis()->SetTitle("Charge [fC]");
  mcharge->GetYaxis()->SetTitleOffset(0.3);
  mcharge->GetYaxis()->SetTitleFont(132);
  mcharge->GetYaxis()->SetTitleSize(0.2);
  mcharge->GetYaxis()->SetLabelSize(0.13);
  mcharge->GetXaxis()->SetRangeUser(min_x-3,max_x+3);
  //mcharge->GetYaxis()->SetNdivisions(130);
  gPad->Update();
